...
export function MomentVideoManager({ moments = [] }) {
  const firstId = moments.find((moment) => moment.id)?.id || "";
  const [selectedId, setSelectedId] = useState(firstId);
  const [videos, setVideos] = useState(() => Object.fromEntries(moments.map((moment) => [moment.id, moment.video || null])));
  const [file, setFile] = useState(null);
  const [busy, setBusy] = useState("");
  const [message, setMessage] = useState("");
  const [error, setError] = useState("");
  const selectedMoment = useMemo(() => moments.find((moment) => moment.id === selectedId) || moments[0] || null, [moments, selectedId]);
  const selectedVideo = videos[selectedId] || null;
  const selectedVideoSize = bytesLabel(selectedVideo?.size);

  async function uploadVideo() {
    if (!selectedId || !file) {
      setError("Choose a moment and a video file first.");
      return;
    }
    ...
    const response = await fetch(`/api/admin/moments/${encodeURIComponent(selectedId)}/video`, {
      method: "POST",
      body: formData,
    });
    ...
    setFile(null);
    setMessage("Video attached to moment.");
    setBusy("");
  }

  async function removeVideo() {
    if (!selectedId) return;
    const confirmed = window.confirm("Remove this moment video? The stored video file will be deleted.");
    if (!confirmed) return;
    ...
    setMessage("Video removed from moment.");
    setBusy("");
  }
  ...
  return (
    <section className="rounded-lg border border-zinc-300 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-amber-700">Moment video</p>
          <h2 className="text-2xl font-black">Attach video to a moment</h2>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-zinc-600">
            Upload, replace, or remove the video attached to a detected moment. Public moment pages can surface the attached video; only admin can change it.
          </p>
        </div>
        <p className="text-sm font-bold text-zinc-500">{moments.filter((moment) => videos[moment.id]).length} attached</p>
      </div>

      <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="grid gap-3">
          <label className="grid gap-2">
            <span className="text-sm font-black">Moment</span>
            <select
          ...
          {selectedMoment ? (
            <div className="rounded-md bg-zinc-100 p-3 text-sm leading-6 text-zinc-700">
              <strong className="text-zinc-950">{selectedMoment.label}</strong>
              {selectedMoment.description ? <p>{selectedMoment.description}</p> : null}
              {selectedVideo ? (
                <p className="mt-2 font-bold">
                  Attached: {selectedVideo.filename}{selectedVideoSize ? ` / ${selectedVideoSize}` : ""}
                </p>
              ) : (
        ...
        <div className="rounded-md border border-zinc-200 bg-zinc-950 p-3 text-white">
          {selectedVideo?.signed_url ? (
            <video key={selectedVideo.signed_url} controls className="aspect-video w-full rounded-sm bg-black" src={selectedVideo.signed_url} />
          ) : (
