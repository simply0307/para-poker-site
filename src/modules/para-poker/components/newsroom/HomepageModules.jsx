...
import { CardGrid, LeagueHero, MomentCard, PlayerCard, SessionCard, StatCard, StatStrip } from "@/modules/para-poker/components/newsroom/NewsroomShell";
import { cleanName, formatDate, formatNumber, text } from "@/modules/para-poker/lib/newsroom/data";
...
function sessionHref(session) {
  return `/para/poker/sessions/${encodeURIComponent(text(session?.session_code || session?.id))}`;
}

function articleHref(article) {
  return `/para/poker/articles/${encodeURIComponent(text(article?.slug || article?.id))}`;
}
    ...
    >
      <div className="flex flex-wrap gap-3">
        {selectedArticle ? (
          ...
          <Link href="/para/poker/standings" className="rounded-sm bg-[#d8c087] px-4 py-3 text-sm font-bold uppercase tracking-[0.12em] text-[#061019]">
            View board
          </Link>
        )}
        <Link href="/para/poker/players" className="rounded-sm border border-[#d8c087]/30 px-4 py-3 text-sm font-bold uppercase tracking-[0.12em] text-[#fff1bf]">
          Player archive
        </Link>
      </div>
    </LeagueHero>
      ...
      <div className={isTopThree ? "grid gap-3 md:grid-cols-3" : "grid gap-2"}>
        {rows.slice(0, isTopThree ? 3 : rows.length).map((row) => (
          <Link
            ...
            href={row.player_id ? `/para/poker/players/${encodeURIComponent(text(row.player_id))}` : "/para/poker/players"}
      ...
      <CardGrid>
        {moments.slice(0, module.variant === "featured_moment" ? 1 : moments.length).map((moment) => (
          <MomentCard
            ...
            href={moment.video?.signed_url ? "" : moment.detailHref || (moment.id ? `/para/poker/moments/${encodeURIComponent(text(moment.id))}` : "")}
          ...
          >
            ...
            {moment.video?.signed_url && (moment.detailHref || moment.id) ? (
              <Link
                href={moment.detailHref || `/para/poker/moments/${encodeURIComponent(text(moment.id))}`}
...
function MomentStrip({ moment }) {
  return (
    <Link
      href={moment.detailHref || (moment.id ? `/para/poker/moments/${encodeURIComponent(text(moment.id))}` : "/para/poker/moments")}
