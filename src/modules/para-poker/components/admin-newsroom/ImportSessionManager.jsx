...
export function ImportSessionManager({ sessions = [] }) {
  ...
  const revisioned = Boolean(selected?.isRevisioned);
    ...
    <section className="mt-8 rounded-lg border border-zinc-300 bg-white p-5 shadow-sm">
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(260px,360px)_minmax(0,1fr)]">
        ...
        <div className="rounded-md border border-zinc-200 p-4">
          {selected ? (
            <>
              <div className={`mb-4 rounded-md border p-3 text-sm ${revisioned ? "border-amber-300 bg-amber-50 text-amber-900" : "border-zinc-200 bg-zinc-50 text-zinc-600"}`}>
                <strong>{selected.evidenceLabel || "legacy unversioned"}.</strong>{revisioned
                  ? " Revisioned sessions are immutable here; use an explicit replacement preview above."
                  : " Legacy unversioned sessions remain available for direct maintenance."}
              </div>
              <div className="grid gap-4 md:grid-cols-3">
                <Input disabled={revisioned} label="Session code" value={form.sessionCode} onChange={(value) => update("sessionCode", value)} />
                <Input disabled={revisioned} label="Season" value={form.seasonCode} onChange={(value) => update("seasonCode", value)} />
                <Input disabled={revisioned} label="Session number" value={form.sessionNumber} onChange={(value) => update("sessionNumber", value)} />
                <Input disabled={revisioned} label="Table name" value={form.tableName} onChange={(value) => update("tableName", value)} />
                <Input disabled={revisioned} label="Format" value={form.format} onChange={(value) => update("format", value)} />
                <Input disabled={revisioned} label="Status" value={form.status} onChange={(value) => update("status", value)} />
                <Input disabled={revisioned} label="Hands count" value={form.handsCount} onChange={(value) => update("handsCount", value)} />
                <Input disabled={revisioned} label="Players count" value={form.playersCount} onChange={(value) => update("playersCount", value)} />
              ...
              <div className="mt-5 flex flex-wrap gap-3">
                <button
                  type="button"
                  onClick={saveSession}
                  disabled={Boolean(busy) || revisioned}
...
function Input({ label, value, onChange, disabled = false }) {
  return (
    <label className="grid gap-2">
      ...
      <input disabled={disabled} value={value || ""} onChange={(event) => onChange(event.target.value)} className="rounded-md border border-zinc-300 px-3 py-2 text-sm disabled:bg-zinc-100 disabled:text-zinc-500" />
    </label>
  );
}
