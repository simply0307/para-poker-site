...
import { ArticleContextSelector } from "@/modules/para-poker/components/admin-newsroom/ArticleContextSelector";
import { PromptConfigPicker } from "@/modules/para-poker/components/admin-newsroom/PromptConfigPicker";
import { RichTextEditor } from "@/modules/para-poker/components/admin-newsroom/RichTextEditor";
import {
  ...
  mergeDraftPayload,
} from "@/modules/para-poker/lib/newsroom/draftTypes";
import { getPromptPreset } from "@/modules/para-poker/lib/newsroom/promptConfigs";
import { stripPlayerHandlesFromText } from "@/modules/para-poker/lib/playerNames";
...
function publicHrefForRow(row = {}) {
  if (row._draft_table === "article_drafts") {
    ...
    return slug ? `/para/poker/articles/${encodeURIComponent(slug)}` : "";
  }
  if (row._draft_table === "moment_blurb_drafts") return "/para/poker/moments";
  if (row._draft_table === "standings_drafts") return "/para/poker/standings";
  if (row._draft_table === "profile_drafts") {
    const playerId = row.player_id || row.source_player_id;
    return playerId ? `/para/poker/players/${encodeURIComponent(playerId)}` : "/para/poker/players";
  }
  if (row._draft_table === "recap_drafts" && row.scope === "session") {
    return row.source_session_id ? `/para/poker/sessions/${encodeURIComponent(row.source_session_id)}` : "/para/poker/sessions";
  }
  return "";
}
