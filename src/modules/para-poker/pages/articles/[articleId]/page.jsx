...
import { ContentRail, EvidencePanel, LeagueHero, NewsroomShell, PublishedArticle, StatCard, StatStrip } from "@/modules/para-poker/components/newsroom/NewsroomShell";
import { getPublishedArticle } from "@/modules/para-poker/lib/newsroom/data";
import { hasRichTextMarkup, sanitizeRichText } from "@/modules/para-poker/lib/newsroom/richText";
...
function articleContextItems(article = {}) {
  ...
  for (const sessionId of selection.sessionIds || selection.session_ids || []) {
    items.push({
      href: `/para/poker/sessions/${encodeURIComponent(sessionId)}`,
      label: sessionId,
      meta: "Session",
    });
  }
  ...
  for (const playerId of selection.playerIds || selection.player_ids || []) {
    items.push({
      href: `/para/poker/players/${encodeURIComponent(playerId)}`,
  ...
  for (const momentId of selection.momentIds || selection.moment_ids || []) {
    items.push({
      href: `/para/poker/moments#${encodeURIComponent(momentId)}`,
      label: momentId,
      meta: "Moment",
    });
  }
