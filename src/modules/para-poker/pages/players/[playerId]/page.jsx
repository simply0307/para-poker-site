...
import {
  ...
  StatCard,
  StatStrip,
} from "@/modules/para-poker/components/newsroom/NewsroomShell";
import {
  formatDate,
  formatNumber,
  formatPokerPercent,
  present,
  text,
} from "@/modules/para-poker/lib/newsroom/data";
import { buildPlayerViewModel } from "@/modules/para-poker/lib/newsroom/viewModels/player";
import { draftHeadline, draftHtml, draftParagraphs, draftSubheadline, waitingCopy } from "@/modules/para-poker/lib/newsroom/rendering";
...
import { PokerStatGrid } from "@/modules/para-poker/components/poker/PokerStatGrid";
import { HandHistoryBlock } from "@/modules/para-poker/components/poker/HandActionLog";
import { formatBb } from "@/modules/para-poker/lib/poker/potUnits";
import { handArchiveItem } from "@/modules/para-poker/lib/poker/handArchiveMetadata";
import { FilterableEvidenceArchive } from "@/modules/para-poker/components/newsroom/FilterableEvidenceArchive";
...
function handDetailHref(hand = {}) {
  ...
  if (!sessionId) return "";
  return `/para/poker/sessions/${encodeURIComponent(text(sessionId))}${hand.hand_no ? `#hand-${hand.hand_no}` : ""}`;
}
      ...
      <section className="mt-12 grid gap-8 pb-12 lg:grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)]">
        <EvidencePanel title="Recent Sessions" empty="No recent session rows are available for this player yet.">
          {recentSessions.map((row, index) => {
            ...
            const href = sessionCode ? `/para/poker/sessions/${encodeURIComponent(text(sessionCode))}` : "";
            return (
