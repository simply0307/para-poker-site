...
import { normalizeHandRow } from "@/modules/para-poker/lib/poker/handHistory";
import { enrichHandWithPotUnits, formatPotWithBb } from "@/modules/para-poker/lib/poker/potUnits";
import { readMomentCurationSettings } from "@/modules/para-poker/lib/newsroom/momentCurationSettings";
import { getMomentVideoAttachments } from "@/modules/para-poker/lib/newsroom/momentVideoAttachments";
...
function enrichMoment(moment, { session, player, publishedDraft, featuredId, topPotCutoff, curationSettings }) {
  ...
  return {
    ...moment,
    momentId,
    detailHref: momentId ? `/para/poker/moments/${encodeURIComponent(momentId)}` : "",
    session,
    sessionCode: text(session?.session_code || moment.session_code || moment.session_id),
    sessionHref: session?.session_code || moment.session_id ? `/para/poker/sessions/${encodeURIComponent(text(session?.session_code || moment.session_id))}` : "",
    player,
    playerName: cleanName(player?.display_name || moment.winner_name, "Winner pending"),
    playerHref: player?.slug || player?.id ? `/para/poker/players/${encodeURIComponent(text(player.slug || player.id))}` : "",
    types,
